...
if [ -n "$DESIRED_KEY_NAMES" ]; then
	...
	for name in $(printf '%s' "$DESIRED_KEY_NAMES" | tr ',' ' '); do
		...
		# `if`, NOT `[ ... ] && printf`. This script runs under
		# `set -e`, and that idiom as the last command of the loop body
		# makes the whole SUBSHELL exit the first time a line does not
		# match -- so the search only ever succeeded for a client that
		# happened to be FIRST in the existing list. For any later one
		# the substitution returned empty AND non-zero, which `set -e`
		# then turned into a silent `exit 1` of the entire script.
		#
		# Concretely: with "dashboard:...;sweep:..." on record, rotating
		# anything worked for dashboard and died on sweep, with no
		# output at all. The documented rotation path
		# (`API_KEY=... ./cluster/chart-secrets.sh`, docs/operations.md)
		# was therefore broken for this cluster's own key list.
		#
		# An `if` always leaves a zero status, so a non-match is just a
		# non-match.
		# printf '%s\n', not '%s': without a trailing newline the LAST
		# entry never reaches the loop body, because `read` returns
		# non-zero at EOF. That was invisible while the `set -e` bug
		# above killed the subshell first, and it is the more dangerous
		# of the two -- a client whose key "could not be found" is
		# silently REGENERATED by the openssl fallback below, so every
		# rotation would quietly re-key whichever consumer happened to
		# be last in the list.
		current="$(printf '%s\n' "$existing_api_keys" | tr ',;' '\n' | while IFS=: read -r n k; do
			if [ "$n" = "$name" ]; then
				printf '%s' "$k"
			fi
		done)"
