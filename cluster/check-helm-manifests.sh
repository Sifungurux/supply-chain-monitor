...
# The dashboard proxy must present the DASHBOARD key, never the master
# one. This is a one-character-class mistake with no visible symptom:
# the nginx config is generated by an UNQUOTED heredoc, so ${API_KEY}
# silently expands to the master key env var while the escaped dashboard
# key sits in a shell variable nothing reads. Everything stays green --
# helm lint, helm template, the dashboard's own tests, and the running
# page -- and the only difference is that every anonymous visitor holds
# unrestricted API authority instead of the scoped read+scan the values
# file says they have.
#
# Found live on 2026-09-15 (report 2026-09-09, S-1, which understated it
# as a fallback rather than the unconditional behaviour it was).
echo "== dashboard proxy presents the dashboard key, not the master key =="
proxy_auth=$(helm template scm-ci charts/supply-chain-monitor \
	-s templates/dashboard/deployment.yaml | grep 'proxy_set_header Authorization' || true)
if [ -z "$proxy_auth" ]; then
	echo "ERROR: no Authorization header found in the dashboard proxy config" >&2
	exit 1
fi
# The single quotes are the point: this matches the LITERAL text
# ${ESCAPED_KEY} in the generated nginx config, which is expanded later
# by the init container's shell, not by this one.
# shellcheck disable=SC2016
case "$proxy_auth" in
	*'${ESCAPED_KEY}'*) ;;
	*)
		echo "ERROR: the dashboard proxy does not present the dashboard key:" >&2
		echo "  $proxy_auth" >&2
		echo "ERROR: expected \${ESCAPED_KEY} (the named 'dashboard' client)." >&2
		echo "ERROR: \${API_KEY} there is the MASTER key -- unrestricted, and inherited" >&2
		echo "ERROR: by anyone who can reach the dashboard." >&2
		exit 1
		;;
esac
