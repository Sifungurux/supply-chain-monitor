...
#
# Taken from our own flag when it was given, NOT from `config
# current-context` -- that subcommand reads the kubeconfig directly and
# IGNORES --context, so asking it would name the default cluster while
# the checks ran against a different one. Which is precisely the
# confusion this line exists to prevent.
if [ -z "$CONTEXT" ]; then
	CONTEXT="$(kubectl config current-context 2>/dev/null || true)"
	[ -n "$CONTEXT" ] || CONTEXT="(none set)"
fi
echo "Checking cluster context: ${CONTEXT}"
...
if ! sed -e "s|PLACEHOLDER_PG_PASSWORD|${LEAKED_PG_PASSWORD}|" \
         -e "s|PLACEHOLDER_API_KEY|${LEAKED_API_KEY}|" \
         "$DIR/check-live-secrets-job.yaml" \
	| "${KUBECTL[@]}" -n "$NAMESPACE" apply -f - >/dev/null; then
	# MUST be caught explicitly. Under `set -e` this would otherwise
	# abort the script with kubectl's own exit status -- and kubectl
	# exits 1 when it cannot reach a cluster, which this script's
	# contract defines as "the credential was ACCEPTED". An unreachable
	# cluster would report itself as a live leak, inverting the one
	# distinction the whole design exists to keep straight.
	echo "INCONCLUSIVE: could not create the check Jobs -- cluster unreachable," >&2
	echo "      context wrong, or namespace '${NAMESPACE}' missing. NOT a pass," >&2
	echo "      and NOT evidence of a live credential either." >&2
	exit 2
fi
