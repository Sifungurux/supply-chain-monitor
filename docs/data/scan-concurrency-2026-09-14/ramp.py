#!/usr/bin/env python3
"""Scan-concurrency ramp on the RACK cluster: raise the cap until it breaks.
...
def main():
    corpus = []
    # Resume: rung 8 completed on the first run before the node-placement
    # filter was fixed. Its numbers are valid -- only scan_jobs_by_node is
    # blank -- so it is kept rather than spending 22 minutes re-measuring
    # a rung that did not break.
    rpath = os.path.join(OUT, "ramp-results.json")
