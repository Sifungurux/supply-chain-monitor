# Supply Chain Monitor — operations manual
...
`architecture.md` for the design and known gaps.
...
architecture.md    design notes, rationale, roadmap
...
`tech-debt-audit.md`'s Status section for what that testing found
...
architecture.md ("Configuring the dashboard via ConfigMap/Secret
...
See architecture.md ("Adding API authentication") for why a
...
- **A second registry credential appears.** Mirroring writes to
  `scm-registry`, and the account monitor-api normally uses is read-only
  -- deliberately, because every scan-worker Job reads those same two
  Secret keys and has no business being able to write to the registry it
  pulls from. Enabling mirroring adds a separate
  `MIRROR_REGISTRY_USERNAME`/`PASSWORD` pair (`scm-writer`) mounted only
  on monitor-api. `cluster/chart-secrets.sh` already generates that
  password; the chart refuses to render without it.

Artifacts you registered before turning this on are backfilled by the
`sweep-registered` CronJob, which gains a pass over scanned artifacts
that have never been mirrored -- paced by `SWEEP_BATCH_SIZE`, so the
fleet drains gradually rather than all at once.
...
turn it off. See architecture.md ("Sweeping registered-but-
...
architecture.md, "Tracking finding lifecycle").
...
`charts/supply-chain-monitor/templates/monitor-api/rbac.yaml` and architecture.md ("Isolating the
...
internally; see architecture.md ("Isolating Trivy scanning") for
...
architecture.md ("Adding Grype as a second CVE scanner") for the
...
architecture.md ("Running monitor-api outside a Kubernetes pod")
...
itself, same as trivy. See architecture.md ("Pluggable scanners")
for the full design.
...
until per-client keys exist — see architecture.md ("Fixed: nothing
...
`architecture.md`, "Isolating the unpack+scan step"), so to
...
loss (see architecture.md, "Normalizing findings and stage
history into their own tables," for exactly how).

See architecture.md ("Swapping the in-memory store for Postgres")
...
— see architecture.md ("Why scm-postgres-backups needed a
...
architecture.md, "Fixed: `scm-postgres-backups` was getting
...
day, on the default schedule). See architecture.md ("Postgres
...
architecture.md, "A single chart for the whole application", for
why it was consolidated). See `architecture.md` for the full
...
- `k8s/releases/supply-chain-monitor-helmrelease.yaml` — the one Flux
  ...
  — see architecture.md, "Fixed: chart template changes never
...
`architecture.md`'s "Ingress: Traefik + Gateway API" for the full
...
See `architecture.md`'s "Known limitations" for the
...
list drifted out of sync with reality (see `tech-debt-audit.md`,
#11).
