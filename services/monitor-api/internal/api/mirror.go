...
// pull-and-push of every layer -- the images this project measures
// against run past 1.5GB (see docs/scan-concurrency-*.md) -- and because
// the alternative to finishing is a partial push that the destination-
// digest check then rejects, leaving the artifact unmirrored anyway.
...
func (h *handler) mirrorArtifact(ctx context.Context, a *artifact.Artifact) *artifact.Artifact {
	...
	local, err := h.mirror.Mirror(mctx, a.Ref, a.Digest)
	if err != nil {
	...
	if err != nil {
		slog.Error("mirrored the artifact but could not persist the rewritten ref",
			"artifact_id", a.ID, "ref", a.Ref, "mirror_ref", local, "err", err)
		return a
	}
