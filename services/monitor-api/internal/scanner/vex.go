...
func ParseVEX(content []byte) ([]artifact.VEXStatement, error) {
	...
	// ponytail: OpenVEX `products`/`subcomponents` are ignored on
	// purpose. A document is uploaded to one artifact through
	// POST /api/v1/artifacts/{id}/vex, so the operator has already said
	// which product this is about -- matching purls against
	// Artifact.Ref/Digest as well would only ever reject statements the
	// uploader meant to apply. If VEX documents ever arrive
	// fleet-wide instead of per-artifact, product matching is what that
	// change needs.
